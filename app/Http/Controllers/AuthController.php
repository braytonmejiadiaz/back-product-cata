...
class AuthController extends Controller
{
    ...
    public function register() {
        ...
        $user->uniqd = uniqid();
        $user->store_name = request()->store_name; // Nombre de la tienda
        $user->slug = Str::slug(request()->store_name);
        ...
        return response()->json([
            'user' => $user,
            'url_tienda' => "http://localhost:4200/{$user->slug}" // Devuelve la URL única
        ], 201);
    }
    ...
    public function update(Request $request) {
        ...
        if ($request->has('store_name') && $request->store_name !== $user->store_name) {
            $request->request->add(['slug' => Str::slug($request->store_name)]);
        }
